...
const createOrderItem = async (req, res) => {
    ...
    if(isAlreadyAdd){
        ...
        const orderItemTemp = await OrderItem.findOne({_id: orderItem._id}).populate({path: 'product', select: 'name price discount'});
...
const getOrderItem = async (req, res) => {
    ...
    const orderItem = await OrderItem.find(objectQuery).populate({path: 'product', select: 'name price discount'});
...
const getSingleOrderItem = async (req, res) => {
    ...
    const orderItem = await OrderItem.findOne({_id: orderItemId}).populate({path: 'product', select: 'name price discount'});
    if(!orderItem){
